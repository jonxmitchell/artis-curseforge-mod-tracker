...
import { Card, CardHeader, CardBody, Button, Input, Switch, Tooltip, Tabs, Tab, Divider, Chip, ScrollShadow, Popover, PopoverTrigger, PopoverContent, CircularProgress, Textarea } from "@nextui-org/react";
import { Paintbrush, Plus, MoveUp, MoveDown, Info, MessageSquare, Eye, FileText, Crown, Bot, Image as ImageIcon, Calendar, RefreshCw, AlertCircle, Check, Trash2 } from "lucide-react";
...
const SaveNotification = ({ status, onClose }) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} className={`fixed bottom-4 right-4 z-50 px-4 py-2 rounded-lg shadow-lg flex items-center gap-2 ${status === "success" ? "bg-success-50 text-success border border-success-200" : "bg-danger-50 text-danger border border-danger-200"}`}>
      {status === "success" ? <Check size={18} /> : <AlertCircle size={18} />}
      <span className="text-sm font-medium">{status === "success" ? "Changes saved successfully" : "Failed to save changes"}</span>
    </motion.div>
  );
};
...
export default function WebhookEditor({ template: initialTemplate, webhook, onSave, isDefault = false }) {
  ...
  return (
    <>
      ...
      <AnimatePresence>{saveStatus && <SaveNotification status={saveStatus} onClose={() => setSaveStatus(null)} />}</AnimatePresence>
    </>
  );
}
